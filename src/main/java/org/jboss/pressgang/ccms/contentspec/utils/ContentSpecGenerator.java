...
public class ContentSpecGenerator<T extends RESTBaseTopicV1<T, U, V>, U extends RESTBaseCollectionV1<T, U, V>,
    ...
    private void populateContentSpecLevel(final U topics, final Level level, final TagRequirements childRequirements,
     ...
     * text based version.
     */
    private ContentSpec doFormattedTocPass(final Class<T> clazz, final U topics, final String locale,
            final DocbookBuildingOptions docbookBuildingOptions) {
        try {
            ...
            if (clazz == RESTTranslatedTopicV1.class) retValue.setLocale(locale);
            ...
            final RESTCategoryV1 technologyCategroy = restClient.getJSONCategory(DocbookBuilderConstants.TECHNOLOGY_CATEGORY_ID,
                    expandString);
            final RESTCategoryV1 commonNamesCategory = restClient.getJSONCategory(DocbookBuilderConstants.COMMON_NAME_CATEGORY_ID,
                    expandString);
            ...
            for (final RESTCategoryV1 category : new RESTCategoryV1[]{technologyCategroy, commonNamesCategory}) {
			...
			/* Get the technology and common names categories */
            final RESTCategoryV1 concernCategory = restClient.getJSONCategory(DocbookBuilderConstants.CONCERN_CATEGORY_ID,
                    concernCategoryExpandString);
            ...
            for (final RESTTagInCategoryV1 tag : topLevelTags) {
				 ...
				 */
                final TagRequirements topLevelBranchTags = new TagRequirements((RESTBaseTagV1<?, ?, ?>) null,
                        new ArrayList<RESTBaseTagV1<?, ?, ?>>() {
                ...
                for (final RESTTagInCategoryV1 concernTag : concernCategory.getTags().returnItems()) {
                    ...
                    final TagRequirements concernLevelDisplayTags = new TagRequirements((RESTBaseTagV1<?, ?, ?>) null,
                            new ArrayList<RESTBaseTagV1<?, ?, ?>>() {
                    ...
                    populateContentSpecLevel(topics, conceptSection, concernLevelChildTags,
                            new TagRequirements(conceptTag, (RESTTagV1) null));
                    populateContentSpecLevel(topics, referenceSection, concernLevelChildTags,
                            new TagRequirements(referenceTag, (RESTTagV1) null));
