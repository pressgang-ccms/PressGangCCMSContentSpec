...
public class DocbookXMLPreProcessor {
        ...
        try {
            ...
            if (topic.getTags() != null && topic.getTags().getItems() != null) {
                ...
                for (final RESTTagV1 tag : tags) {
                    final RESTAssignedPropertyTagV1 bugzillaProductTag = ComponentBaseRESTEntityWithPropertiesV1.returnProperty(tag,
                            CommonConstants.BUGZILLA_PRODUCT_PROP_TAG_ID);
                    final RESTAssignedPropertyTagV1 bugzillaComponentTag = ComponentBaseRESTEntityWithPropertiesV1.returnProperty(tag,
                            CommonConstants.BUGZILLA_COMPONENT_PROP_TAG_ID);
                    final RESTAssignedPropertyTagV1 bugzillaKeywordsTag = ComponentBaseRESTEntityWithPropertiesV1.returnProperty(tag,
                            CommonConstants.BUGZILLA_KEYWORDS_PROP_TAG_ID);
                    final RESTAssignedPropertyTagV1 bugzillaVersionTag = ComponentBaseRESTEntityWithPropertiesV1.returnProperty(tag,
                            CommonConstants.BUGZILLA_VERSION_PROP_TAG_ID);
                    final RESTAssignedPropertyTagV1 bugzillaAssignedToTag = ComponentBaseRESTEntityWithPropertiesV1.returnProperty(tag,
                            CommonConstants.BUGZILLA_PROFILE_PROPERTY);
    ...
    public void processTopicAdditionalInfo(final SpecTopic specTopic, final Document document, final BugzillaOptions bzOptions,
        ...
        if ((docbookBuildingOptions != null && (docbookBuildingOptions.getInsertSurveyLink() || docbookBuildingOptions
            ...
            if (docbookBuildingOptions != null && docbookBuildingOptions.getInsertEditorLinks()) {
                final String editorUrl = topic instanceof RESTTopicV1 ? ComponentTopicV1.returnEditorURL(
                        (RESTTopicV1) topic) : ComponentTranslatedTopicV1.returnEditorURL((RESTTranslatedTopicV1) topic, zanataDetails);
    ...
    public <T extends RESTBaseTopicV1<T, ?, ?>> List<Integer> processInjections(final Level level, final SpecTopic topic,
        ...
        errorTopics.addAll(processInjections(level, topic, customInjectionIds, customInjections, ITEMIZEDLIST_INJECTION_POINT, xmlDocument,
                CUSTOM_ALPHA_SORT_INJECTION_LIST_RE, new TopicTitleSorter<T>(), docbookBuildingOptions, relatedTopicDatabase,
                usedFixedUrls));
