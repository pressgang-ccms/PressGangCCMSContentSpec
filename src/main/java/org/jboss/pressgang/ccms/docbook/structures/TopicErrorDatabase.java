...
public class TopicErrorDatabase<T extends RESTBaseTopicV1<T, ?, ?>> {
                ...
                if (ComponentBaseTopicV1.returnIsDummyTopic(topic)) {
                    if (topic.getClass() == RESTTranslatedTopicV1.class && topicErrorData.getTopic() instanceof RESTTranslatedTopicV1) {
                        if (((RESTTranslatedTopicV1) topicErrorData.getTopic()).getTopicId().equals(
                                ((RESTTranslatedTopicV1) topic).getTopicId())) return topicErrorData;
                    }
                } else {
                    if (topicErrorData.getTopic().getId().equals(topic.getId())) return topicErrorData;
                }
            }
        return null;
    }
    ...
    private TopicErrorData<T> addOrGetTopicErrorData(final T topic) {
        ...
        if (topicErrorData == null) {
            ...
            if (!errors.containsKey(topic.getLocale())) errors.put(topic.getLocale(), new ArrayList<TopicErrorData<T>>());
